...
for ThisObsClass=1:3
    ...
    col(1,:)=hsv2rgb([.33 1/ThisObsClass .5]);
    col(2,:)=hsv2rgb([0 .6/ThisObsClass .9]);
    col(3,:)=hsv2rgb([0 0 0]);
    col(4,:)=hsv2rgb([.75 1/ThisObsClass .8]);
    col(5,:)=hsv2rgb([.66 .7/ThisObsClass 1]);
    col(6,:)=hsv2rgb([.7 .5/ThisObsClass .9]);
    col(7,:)=hsv2rgb([.33 .5/ThisObsClass .8]);
    col(8,:)=hsv2rgb([.33 .6/ThisObsClass 1]); % first T
    col(9,:)=hsv2rgb([.33 .6/ThisObsClass .5]); % last T
    col(10,:)=hsv2rgb([.45 .6/ThisObsClass .4]); % other
    col(11,:)=hsv2rgb([.25 1/ThisObsClass .9]); % out2in
    ...
    olines=[];
	for NObs=1:ObsN
        olines=[olines transpose(find(UFOVmammo.participant==NObs))];
    end
